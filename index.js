...
function addArtist(array, name, years, genre, nationality, bio) {
  const oneMoreArtist = [... array]

oneMoreArtist.push({name, years, genre, nationality, bio})

return oneMoreArtist
}
console.log(addArtist(artists, 'Jose Herrera', '1997 - present', 'any and everything', 'Colombian', 'Supa dope kid'))
...
function lotsOfArt(array) {
  const stayingBusy = []
  const overOneHundred = artists.filter (element => element.paintings > 100)
  
 for (let i = 0; i < overOneHundred.length; i++){
  stayingBusy.push(overOneHundred[i].name)
}
  return stayingBusy
}
...
function artistByCountry(array, nationality){
  const patriotism = []
  const artistsByCountry = artists.filter (element => element.nationality === nationality)

  for (let i = 0; i < artistsByCountry.length; i++){
  patriotism.push(artistsByCountry[i].name)
}
return patriotism
}
console.log(artistByCountry(artists, 'French'))
